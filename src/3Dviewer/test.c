...
START_TEST(scan_obj_test) {
  ...
  size_obj("./objects/cube_test2.obj", &count_v, &count_f, &v_in_f);
  struct_malloc(&data);
  scan_obj(&data, "./objects/cube_test2.obj");

  ck_assert_int_eq(count_f, 13);
  ck_assert_int_eq(count_v, 8);
  ck_assert_int_eq(v_in_f, 3);
  ...
  ck_assert_str_eq(buf2, "-1.000000");
